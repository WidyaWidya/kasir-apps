...
export const metadata: Metadata = {
  title: "Master Merk | Kasir App",
  description: "Daftar master merk produk",
};
...
export default async function MerkPage({
  ...
  const response = await getBrands({ search, sortBy: sort });
  const brands = response.success && response.data ? response.data : [];

  return (
    <div>
      <PageBreadcrumb pageTitle="MASTER MERK" />
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/merk/add"
              className="rounded-lg bg-green-500 px-4 py-2 text-sm font-medium text-white hover:bg-green-600 transition-colors flex items-center gap-1.5"
            >
