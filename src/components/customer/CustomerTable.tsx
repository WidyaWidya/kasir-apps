...
export default function CustomerTable({ customers }: { customers: Customer[] }) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    if (res.success) {
      ...
      alert(res.error || "Gagal menghapus pelanggan");
    }
  };
            ...
            {customers.map((customer) => (
              <TableRow key={customer.id}>
                ...
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400 max-w-xs">
                  {customer.address || <span className="text-gray-300 dark:text-gray-600 italic">—</span>}
                </TableCell>
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                  {customer.phone || <span className="text-gray-300 dark:text-gray-600 italic">—</span>}
                </TableCell>
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                  {customer.email || <span className="text-gray-300 dark:text-gray-600 italic">—</span>}
                </TableCell>
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400 max-w-xs">
                  {customer.description || <span className="text-gray-300 dark:text-gray-600 italic">—</span>}
                </TableCell>
                <TableCell className="px-4 py-3 text-center text-sm font-medium">
                  <span className={customer.isActive ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}>
                    {customer.isActive ? "Aktif" : "Nonaktif"}
                  </span>
                </TableCell>
                <TableCell className="px-4 py-3 text-end">
                  <div className="flex items-center justify-end gap-3">
                    <Link href={`/pelanggan/${customer.id}`} className="text-brand-500 hover:text-brand-600 dark:text-brand-400 dark:hover:text-brand-300 font-medium text-sm transition-colors flex items-center gap-1" title="Edit">
